...
void SCS_HTTP_JSYS_CONTROL::controlSend(string _id, int _val)
{
	...
	{
		memset(m_log_buf_, 0, sizeof(m_log_buf_));
		sprintf(m_log_buf_, "[%s] INFO controlSend() 控制指令下发成功！ line[%d]!", m_pGroup->name, __LINE__);
		FepLog::writelog(m_pGroup->name, m_pLink->name, m_log_buf_, FEP_LOG::PKGDATA);
	}
	else
	{
...
bool SCS_HTTP_JSYS_CONTROL::readProtocolParam(string _param) //获取协议参数
{
	memset(m_log_buf_, 0, sizeof(m_log_buf_));
...
bool SCS_HTTP_JSYS_CONTROL::sendCtrlCmd() //处理控制命令
{
