...
bool getTag(MYSQL *_mysql, map<string, TAGINFO> *_tagMap, const void *_groupName, hInt32 _groupNo, const void *_linkName)
{
	...
	strGroupNo = groupbuf;
	sql = "SELECT ValueType AS tagType, OffsetNo AS offset, Param1, MaximumValue, MinimumValue, ValueRadio, ValueOffset, ValueDeaband, Name ,Param2 FROM cfg_view_point_protocol WHERE GroupNo ='" + strGroupNo + "' and OffsetNo is not null ORDER BY Param1;";
	...
	while ((row = mysql_fetch_row(res)))
	{
		...
		memset(global_logbuff, 0, sizeof(global_logbuff));
		sprintf(global_logbuff, "[DB] INFO getTag() TagInfo tagName:%s offset:%d valueType:%d maxValue:%d minValue:%d valueRadio:%d valueOffset:%d deaband:%d Param1:%s Param2:%s line[%d]!",
