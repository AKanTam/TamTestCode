#include "scs_ws_center_supcon.h"
...
bool SCS_WS_CENTER_SUPCON::open()
{
	...
	webSocketUrl = "ws://" + m_remoteAddress + "/socket/001";
	...
	else
	{
		if (webSocket->getReadyState() == WebSocket::OPEN)
		{
			...
			if (saveFlag)
			{
				...
				saveFlag = false;
				memset(m_log_buf_, 0, sizeof(m_log_buf_));
				sprintf(m_log_buf_, "[%s] INFO run() 轮询任务  url[%s] route[%d] mainRoute[%d] m_link[%d] line[%d]!",
						m_pGroup->name, webSocketUrl.c_str(), m_curRoute, (hBool)m_pRouteInfo->mainFlag, m_link, __LINE__);
				FepLog::writelog(m_pGroup->name, m_pLink->name, m_log_buf_, FEP_LOG::PKGDATA);
			}
...
static void refreshValue(std::string &_tagName, cJSON *_item, const char *_name1, const char *_name2, std::map<int, std::map<std::string, TAGINFO>>::iterator &_mapIt, std::map<std::string, TAGINFO>::iterator &_tagIt) //单层json解析
{
	_tagName.clear();
	cJSON *_name1_json = cJSON_GetObjectItemCaseSensitive(_item, _name1);
	if (cJSON_IsNumber(_name1_json))
	{
		_tagName = std::to_string(_name1_json->valueint) + "_";
		_tagName += _name2;
	}
	else if (cJSON_IsString(_name1_json) && (_name1_json->valuestring != NULL))
	{
		_tagName = _name1_json->valuestring;
		_tagName += "_";
		_tagName += _name2;
	}
	else
	...
	itemTag = cJSON_GetObjectItemCaseSensitive(_item, _name2);

	if (itemTag != NULL)
	{
		_tagIt = _mapIt->second.find(_tagName);
		if (_tagIt != _mapIt->second.end())
		{
			_tagIt->second.updateTime = updateTime;
			if (cJSON_IsNumber(itemTag))
			{
				_tagIt->second.value = itemTag->valuedouble;
			}
			else if (cJSON_IsString(itemTag) && (itemTag->valuestring != NULL))
			{
				_tagIt->second.value_s = itemTag->valuestring;
			}
		}
	}
...
static void refreshValueFromObject(std::string &_tagName, cJSON *_item, const char *_name1, const char *_name_obj, std::map<int, std::map<std::string, TAGINFO>>::iterator &_mapIt, std::map<std::string, TAGINFO>::iterator &_tagIt) //二层json对象解析
{
	std::string _saveName;
	cJSON *_name1_json = cJSON_GetObjectItemCaseSensitive(_item, _name1);
	cJSON *_name_obj_json = cJSON_GetObjectItemCaseSensitive(_item, _name_obj);
	if (cJSON_IsObject(_name_obj_json))
	{
		_saveName.clear();
		if (cJSON_IsNumber(_name1_json))
		{
			_saveName = std::move(std::to_string(_name1_json->valueint));
			_saveName += "_";
			_saveName += _name_obj_json->string;
			_saveName += "_";
		}
		else if (cJSON_IsString(_name1_json) && (_name1_json->valuestring != NULL))
		{
			_saveName = _name1_json->valuestring;
			_saveName += "_";
			_saveName += _name_obj_json->string;
			_saveName += "_";
		}
	...
	cJSON_ArrayForEach(itemTag, _name_obj_json)
	{
		_tagName.clear();
		_tagName = _saveName.data();
		_tagName += itemTag->string;
...
void handle_message(const std::string &message)
{
	...
	cJSON *mapType = cJSON_GetObjectItemCaseSensitive(json, "type");
	cJSON *data = cJSON_GetObjectItemCaseSensitive(json, "data");
	cJSON *item = NULL;

	std::string tagName;
	auto mapIt = m_webSocketTagMap.end();
	auto tagIt = mapIt->second.end();
	updateTime = time(0);
	...
	if (cJSON_IsNumber(mapType) && cJSON_IsArray(data))
	{
		mapIt = m_webSocketTagMap.find(mapType->valueint);
		if (mapIt == m_webSocketTagMap.end())
		{
			cJSON_Delete(json);
			return;
		}
		...
		{
			switch (mapType->valueint)
			{
			case JIATONG_DICI:
			{
				cJSON_ArrayForEach(item, data)
				{
					refreshValue(tagName, item, "serial_number", "status", mapIt, tagIt);
					refreshValue(tagName, item, "serial_number", "online", mapIt, tagIt);
				}
				break;
			}
			...
			case JIATONG_KELIU:
			{
				cJSON_ArrayForEach(item, data)
				{
					refreshValue(tagName, item, "id", "dayTotalEnterNum", mapIt, tagIt);
					refreshValue(tagName, item, "id", "dayTotalOutNum", mapIt, tagIt);
					refreshValue(tagName, item, "id", "residentNum", mapIt, tagIt);
					refreshValue(tagName, item, "id", "queuedIndex", mapIt, tagIt);
					refreshValue(tagName, item, "id", "fiveMinTotalOutNum", mapIt, tagIt);
					refreshValue(tagName, item, "id", "fiveMinTotalEnterNum", mapIt, tagIt);
					refreshValue(tagName, item, "id", "fiveResident", mapIt, tagIt);
				}
				break;
			}

			case JIASHAODAQIAO_CHELIU:
			{
				cJSON_ArrayForEach(item, data)
				{
					refreshValue(tagName, item, "equipmenNo", "startTime", mapIt, tagIt);
					refreshValue(tagName, item, "equipmenNo", "endTime", mapIt, tagIt);
					refreshValueFromObject(tagName, item, "equipmenNo", "sectionFlow", mapIt, tagIt);
					refreshValueFromObject(tagName, item, "equipmenNo", "allLane", mapIt, tagIt);
				}
				break;
			}
