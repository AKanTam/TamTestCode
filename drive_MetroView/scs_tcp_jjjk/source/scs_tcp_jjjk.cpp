#include "SCS_TCP_JJJK.h"

using namespace ECON::FDC;

#define LOGE_TCP_JJJK 31002
...
SCS_TCP_JJJK::SCS_TCP_JJJK() : m_logd_id_(LOGE_TCP_JJJK)
{
	m_isOpen = false;
	m_isload = false;
	m_poll_tag_num_ = 100;
...
SCS_TCP_JJJK::~SCS_TCP_JJJK()
{
}

bool SCS_TCP_JJJK::isOpen() const
{
	return m_isOpen;
}
bool SCS_TCP_JJJK::open()
{
	m_isOpen = CProtocol::open();
	sprintf(m_log_buf_, "%s", m_pLink->remoteAddr); //(本地地址为m_pLink->localAddr)
...
bool SCS_TCP_JJJK::loadTag(map<string, TAGINFO> *_readMap, map<string, TAGINFO> *_writeMap)
{
...
bool SCS_TCP_JJJK::readProtocolParam(string _param) //获取协议参数
{
	memset(m_log_buf_, 0, sizeof(m_log_buf_));
...
void SCS_TCP_JJJK::ProtocolTX() //发送请求报文
{
	// if (!checkSend())
	// {
	// 	return;
...
void SCS_TCP_JJJK::ProtocolRX() //数据上送
{

	procData();
}

bool SCS_TCP_JJJK::checkSend()
{
	if (!m_send_flag)
	{
		m_try_send_times_ = 0;
...
void SCS_TCP_JJJK::Send_CallAllData() //轮询请求数据
{
	...
	if (!tcpJJJKSubPoll(m_remoteAddress, &m_subDataMap, m_pGroup->name, m_pRoute->group, m_pLink->name))
	{
		memset(m_log_buf_, 0, sizeof(m_log_buf_));
...
bool SCS_TCP_JJJK::sendCtrlCmd() //处理控制命令
{
...
void SCS_TCP_JJJK::saveYxData(TAGINFO _tag)
{
	FDC_YX_DATA yx_data;
...
void SCS_TCP_JJJK::saveYcData(TAGINFO _tag)
{
	FDC_YC_DATA yc_data;
...
void SCS_TCP_JJJK::saveKwhData(TAGINFO _tag)
{
...
void SCS_TCP_JJJK::saveStringData(TAGINFO _tag)
{
...
// //----------------------------------------------------------------------------
extern "C" TCP_JJJK_EXPORT ECON::FDC::CProtocol *CreateProtocol()
{
	return (new SCS_TCP_JJJK());
}

extern "C" TCP_JJJK_EXPORT void DestroyProtocol(ECON::FDC::CProtocol *p)
{
	delete p;
}
