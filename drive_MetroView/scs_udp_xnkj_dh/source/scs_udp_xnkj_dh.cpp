#include "SCS_UDP_XNKJ_DH.h"

using namespace ECON::FDC;

#define LOGE_UDP_XNKJ_DH 31001
...
SCS_UDP_XNKJ_DH::SCS_UDP_XNKJ_DH() : m_logd_id_(LOGE_UDP_XNKJ_DH)
{
	m_isOpen = false;
	m_isload = false;
	m_poll_tag_num_ = 100;
...
SCS_UDP_XNKJ_DH::~SCS_UDP_XNKJ_DH()
{
}

bool SCS_UDP_XNKJ_DH::isOpen() const
{
	return m_isOpen;
}
bool SCS_UDP_XNKJ_DH::open()
{
...
void SCS_UDP_XNKJ_DH::run()
{
	// ProtocolTX(); //发送请求报文
	//  ec_usleep(SLEEP_TIME); //发包和收包间隔，因为不同厂家设备响应速度不同，要综合考虑参数设置
	// ProtocolRX(); //接收请求报文、解析并上送数据

	if (!udpXnkjDhSubPoll(&m_subDataMap, m_pGroup->name, m_pRoute->group, m_pLink->name))
	{
		// memset(m_log_buf_, 0, sizeof(m_log_buf_));
		// sprintf(m_log_buf_, "[%s] INFO Send_CallAllData() 查询任务失败 line[%d]", m_pGroup->name, __LINE__);
		// FepLog::writelog(m_pGroup->name, m_pLink->name, m_log_buf_, FEP_LOG::PKGDATA);
	}
	else
	{
		this->procData();
		// memset(m_log_buf_, 0, sizeof(m_log_buf_));
		// sprintf(m_log_buf_, "[%s] INFO Send_CallAllData() 查询点任务完成 line[%d]", m_pGroup->name, __LINE__);
		// FepLog::writelog(m_pGroup->name, m_pLink->name, m_log_buf_, FEP_LOG::PKGDATA);
	}
...
bool SCS_UDP_XNKJ_DH::loadTag(map<string, TAGINFO> *_readMap, map<string, TAGINFO> *_writeMap)
{
...
bool SCS_UDP_XNKJ_DH::readProtocolParam(string _param) //获取协议参数
{
	memset(m_log_buf_, 0, sizeof(m_log_buf_));
...
void SCS_UDP_XNKJ_DH::ProtocolTX() //发送请求报文
{
	// if (!checkSend())
	// {
	// 	return;
...
void SCS_UDP_XNKJ_DH::ProtocolRX() //数据上送
{

	procData();
}

bool SCS_UDP_XNKJ_DH::checkSend()
{
	if (!m_send_flag)
	{
		m_try_send_times_ = 0;
...
void SCS_UDP_XNKJ_DH::Send_CallAllData() //轮询请求数据
{

	m_send_flag = true;
}

void SCS_UDP_XNKJ_DH::procData()
{
	memset(m_log_buf_, 0, sizeof(m_log_buf_));
	...
	if (m_subDataMap.empty())
	{
		memset(m_log_buf_, 0, sizeof(m_log_buf_));
		sprintf(m_log_buf_, "[%s] INFO procData() 没有接收到数据 line[%d]", m_pGroup->name, __LINE__);
		FepLog::writelog(m_pGroup->name, m_pLink->name, m_log_buf_, FEP_LOG::PKGDATA);
	}

	// for (map<string, float>::iterator itData = m_subDataMap.begin(); itData != m_subDataMap.end(); itData++)
	// {
	// 	map<string, TAGINFO>::iterator itreadlist = m_readTagMap.find(itreadlist->first);
	// 	if (itreadlist != m_readTagMap.end())
	// 	{
	// 		//上送实时库
	...
	// 		case TAG_TYPE_YX:
	// 		{
	// 			itreadlist->second.value = itData->second;
	...
	// 			itreadlist->second.value_s = itData->second;
	...
	// 	}
	// }
	map<string, float>::iterator itData;
	for (map<string, TAGINFO>::iterator itreadlist = m_readTagMap.begin(); itreadlist != m_readTagMap.end(); itreadlist++)
	{
		itData = m_subDataMap.find(itreadlist->second.name);
		if (itData == m_subDataMap.end())
		{
			continue;
		}
		//上送实时库
		...
		case TAG_TYPE_YX:
		{
			itreadlist->second.value = m_subDataMap[itreadlist->second.name];
			saveYxData(itreadlist->second);
			break;
		}
		case TAG_TYPE_YC:
		...
		case TAG_TYPE_KWH:
			...
			break;
		case TAG_TYPE_STRING:
			itreadlist->second.value_s = m_subDataMap[itreadlist->second.name];
			saveStringData(itreadlist->second);
			break;
		default:
			break;
...
bool SCS_UDP_XNKJ_DH::sendCtrlCmd() //处理控制命令
{
...
void SCS_UDP_XNKJ_DH::saveYxData(TAGINFO _tag)
{
	FDC_YX_DATA yx_data;
...
void SCS_UDP_XNKJ_DH::saveYcData(TAGINFO _tag)
{
	FDC_YC_DATA yc_data;
...
void SCS_UDP_XNKJ_DH::saveKwhData(TAGINFO _tag)
{
...
void SCS_UDP_XNKJ_DH::saveStringData(TAGINFO _tag)
{
...
// //----------------------------------------------------------------------------
extern "C" UDP_XNKJ_DH_EXPORT ECON::FDC::CProtocol *CreateProtocol()
{
	return (new SCS_UDP_XNKJ_DH());
}

extern "C" UDP_XNKJ_DH_EXPORT void DestroyProtocol(ECON::FDC::CProtocol *p)
{
	delete p;
}
