...
bool getTag(MYSQL *_mysql, map<string, TAGINFO> *_tagMap, const void *_groupName, hInt32 _groupNo, const void *_linkName)
{
	...
	while ((row = mysql_fetch_row(res)))
	{
		...
		memset(global_logbuff, 0, sizeof(global_logbuff));
		sprintf(global_logbuff, "[DB] INFO getTag() TagInfo name:%s offset:%d valueType:%d maxValue:%d minValue:%d valueRadio:%d valueOffset:%d deaband:%d  pointName:%s  line[%d]!",
				temp_tag.name.c_str(), temp_tag.offset, temp_tag.valuetype, temp_tag.maxValue, temp_tag.minValue, temp_tag.valueRadio, temp_tag.valueOffset, temp_tag.deaband, temp_tag.pointName.c_str(), __LINE__);
		FepLog::writelog((char *)_groupName, (char *)_linkName, global_logbuff, FEP_LOG::PKGDATA);
	}
