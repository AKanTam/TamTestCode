...
bool getTag(MYSQL* _mysql, map<string, TAGINFO> *_tagMap, const void* _groupName, hInt32 _groupNo, const void* _linkName) {
	...
	strGroupNo = groupbuf;
	sql = "SELECT ValueType AS tagType, OffsetNo AS offset, Param1 AS opcTagName, MaximumValue, MinimumValue, ValueRadio, ValueOffset, ValueDeaband, Name FROM cfg_view_point_protocol WHERE GroupNo ='" + strGroupNo + "' and OffsetNo is not null ORDER BY tagType, offset;";
	...
	while((row = mysql_fetch_row(res))) {
		...
		memset(global_logbuff, 0, sizeof(global_logbuff));
		sprintf(global_logbuff,( "[DB] INFO getTag() TagInfo tag1:%s offset:%d valueType:%d maxValue:%d minValue:%d valueRadio:%d valueOffset:%d deaband:%d  pointName:%s  line[%d]!",\
			temp_tag.tag1.c_str()), temp_tag.offset, temp_tag.valuetype, temp_tag.maxValue , temp_tag.minValue, temp_tag.valueRadio, temp_tag.valueOffset, temp_tag.deaband,temp_tag.pointName.c_str(),__LINE__);
		FepLog::writelog((char*)_groupName, (char*)_linkName, global_logbuff, FEP_LOG::PKGDATA);
	}
...
bool getCmd(MYSQL* _mysql, map<string, TAGINFO> *_tagMap, const void* _groupName, hInt32 _groupNo, const void* _linkName) {
	...
	sprintf(groupbuf, "%d", _groupNo);
	strGroupNo = groupbuf;
	sql = "SELECT ValueType AS tagType, Param1 AS opcTagName, MaximumValue, MinimumValue, ValueRadio, ValueOffset, ValueDeaband ,`Name` FROM cfg_view_cmd_protocol WHERE GroupNo ='" + strGroupNo + "'  ORDER BY tagType;";
	if (0 != mysql_query(_mysql, sql.c_str())) {
		memset(global_logbuff, 0, sizeof(global_logbuff));
