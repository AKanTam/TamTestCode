#include "scs_http_jxServer_light.h"

#include "httplib.h"

using namespace ECON::FDC;
#define LOGE_HTTP_JXSERVER_LIGHT 31009

using namespace httplib;
httplib::Client cli("http://xmgl.jianweisoftware.com:9002");
const char *str_poll = "{\"projectKey\":\"JXFWQ\"}";
...
SCS_HTTP_JXSERVER_LIGHT::SCS_HTTP_JXSERVER_LIGHT() : m_logd_id_(LOGE_HTTP_JXSERVER_LIGHT)
{
	m_isOpen = false;
	m_isload = false;
	m_poll_tag_num_ = 100;
...
SCS_HTTP_JXSERVER_LIGHT::~SCS_HTTP_JXSERVER_LIGHT()
{
}

bool SCS_HTTP_JXSERVER_LIGHT::createReadTask()
{
	...
	{
		dataMap.insert(std::make_pair(it->second.param1, it->second));
	}

	return true;
}

bool SCS_HTTP_JXSERVER_LIGHT::isOpen() const
{
	return m_isOpen;
}
bool SCS_HTTP_JXSERVER_LIGHT::open()
{
...
void SCS_HTTP_JXSERVER_LIGHT::close()
{
	DoneRead();
	//通用处理
	CProtocol::close();
...
void SCS_HTTP_JXSERVER_LIGHT::run()
{
	if(!sendCtrlCmd()){
	if (auto resPoll = cli.Post("/smart-api/open/api/device-state", str_poll, "application/json"))
	{
		cJSON *json = cJSON_Parse(resPoll->body.c_str());
		hUInt32 updateTime = time(0);
		if (json == NULL)
		{
			memset(m_log_buf_, 0, sizeof(m_log_buf_));
			sprintf(m_log_buf_, "[%s] ERROR run() 解析json失败 line[%d]!", m_pGroup->name, __LINE__);
			FepLog::writelog(m_pGroup->name, m_pLink->name, m_log_buf_, FEP_LOG::PKGDATA);
			return;
		}
		...
		if (cJSON_IsNumber(code) && (code->valueint == 0))
		{
			if (cJSON_IsArray(result))
			{
				...
				{
					deviceId = cJSON_GetObjectItemCaseSensitive(item, "deviceId");
					value = cJSON_GetObjectItemCaseSensitive(item, "value");
					param = cJSON_GetObjectItemCaseSensitive(item, "param");
					if (cJSON_IsNumber(param) && cJSON_IsNumber(value) && cJSON_IsNumber(deviceId))
					{
						if (param->valueint == 1)
						{
							auto it = dataMap.find(std::to_string(deviceId->valueint));
							if (it != dataMap.end())
							{
								int cout = dataMap.count(std::to_string(deviceId->valueint));
								for (int i = 0; i != cout; i++, it++)
								{
									it->second.value = value->valueint;
		...
		else
		{
			...
			{
				memset(m_log_buf_, 0, sizeof(m_log_buf_));
				sprintf(m_log_buf_, "[%s] ERROR run() 请求失败 (%s) line[%d]!", m_pGroup->name, message->valuestring, __LINE__);
				FepLog::writelog(m_pGroup->name, m_pLink->name, m_log_buf_, FEP_LOG::PKGDATA);
			}
			else
			{
				memset(m_log_buf_, 0, sizeof(m_log_buf_));
				sprintf(m_log_buf_, "[%s] ERROR run() 请求失败 line[%d]!", m_pGroup->name, __LINE__);
				FepLog::writelog(m_pGroup->name, m_pLink->name, m_log_buf_, FEP_LOG::PKGDATA);
			}
		}
		cJSON_Delete(json);
	...
	{
		auto err = resPoll.error();
		memset(m_log_buf_, 0, sizeof(m_log_buf_));
		sprintf(m_log_buf_, "[%s] ERROR run() 连接服务器失败 (%s) line[%d]!", m_pGroup->name, httplib::to_string(err).c_str(), __LINE__);
		FepLog::writelog(m_pGroup->name, m_pLink->name, m_log_buf_, FEP_LOG::PKGDATA);
	}
	}
...
void SCS_HTTP_JXSERVER_LIGHT::controlSend(string _id, int _val)
{
	std::string str_ctrl = "{\"deviceId\":\"";
	str_ctrl += _id;
	str_ctrl += "\",\"param\":1,\"value\":";
	str_ctrl += std::to_string(_val);
	str_ctrl += "}";
	if (auto resCtrl = cli.Post("/smart-api/open/api/device-control", str_ctrl, "application/json"))
	{
		cJSON *json = cJSON_Parse(resCtrl->body.c_str());
		cJSON *message = cJSON_GetObjectItemCaseSensitive(json, "message");

		if (cJSON_IsString(message) && message->valuestring != NULL)
		{
			memset(m_log_buf_, 0, sizeof(m_log_buf_));
			sprintf(m_log_buf_, "[%s] INFO controlSend() %s line[%d]!", m_pGroup->name, UtfToGbk(message->valuestring).c_str(), __LINE__);
			FepLog::writelog(m_pGroup->name, m_pLink->name, m_log_buf_, FEP_LOG::PKGDATA);
		}
		else
		{
			memset(m_log_buf_, 0, sizeof(m_log_buf_));
			sprintf(m_log_buf_, "[%s] ERROR controlSend() 控制指令返回失败！ line[%d]!", m_pGroup->name, __LINE__);
			FepLog::writelog(m_pGroup->name, m_pLink->name, m_log_buf_, FEP_LOG::PKGDATA);
		}
	}
	else
	{
		...
		memset(m_log_buf_, 0, sizeof(m_log_buf_));
		sprintf(m_log_buf_, "[%s] ERROR controlSend() 连接服务器失败 (%s) line[%d]!", m_pGroup->name, httplib::to_string(err).c_str(), __LINE__);
		FepLog::writelog(m_pGroup->name, m_pLink->name, m_log_buf_, FEP_LOG::PKGDATA);
	}
}

bool SCS_HTTP_JXSERVER_LIGHT::loadTag(map<string, TAGINFO> *_readMap, map<string, TAGINFO> *_writeMap)
{
...
bool SCS_HTTP_JXSERVER_LIGHT::readProtocolParam(string _param) //获取协议参数
{
	memset(m_log_buf_, 0, sizeof(m_log_buf_));
...
void SCS_HTTP_JXSERVER_LIGHT::procData()
{
...
bool SCS_HTTP_JXSERVER_LIGHT::sendCtrlCmd() //处理控制命令
{
...
void SCS_HTTP_JXSERVER_LIGHT::saveFEPData(const TAGINFO &_tag)
{
...
void SCS_HTTP_JXSERVER_LIGHT::saveYxData(const TAGINFO &_tag)
{
	FDC_YX_DATA yx_data;
...
void SCS_HTTP_JXSERVER_LIGHT::saveYcData(const TAGINFO &_tag)
{
	FDC_YC_DATA yc_data;
...
void SCS_HTTP_JXSERVER_LIGHT::saveKwhData(const TAGINFO &_tag)
{
...
void SCS_HTTP_JXSERVER_LIGHT::saveStringData(const TAGINFO &_tag)
{
...
// //----------------------------------------------------------------------------
extern "C" HTTP_JXSERVER_LIGHT_EXPORT ECON::FDC::CProtocol *CreateProtocol()
{
	return (new SCS_HTTP_JXSERVER_LIGHT());
}

extern "C" HTTP_JXSERVER_LIGHT_EXPORT void DestroyProtocol(ECON::FDC::CProtocol *p)
{
	delete p;
}
