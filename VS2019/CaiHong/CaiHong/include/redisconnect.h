...
public:
	...
	static shared_ptr<RedisConnect> Instance()
	{
		...
		auto get = [&]()
		{
			if (redis)
			{
				if (redis->ping() > 0 || redis->reconnect())
					return true;
			}
			else
			{
		...
		if (get())
		{
			...
			if (datmap.size() < POOL_MAXLEN)
				datmap.insert(pair<shared_ptr<RedisConnect>, time_t>(redis, now));
		}
